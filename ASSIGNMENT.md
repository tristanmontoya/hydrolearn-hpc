...
The activity files, scripts, data, and a list of required software tools are available in the [HydroLearn HPC GitHub repository](https://github.com/tristanmontoya/hydrolearn-hpc). You can run the examples on the [virtual HPC cluster](https://github.com/tristanmontoya/vhpc-hydrotools), where the required tools are pre-installed, or on another Slurm cluster with the required software installed. If installing the required software on your own system, we strongly recommend that you use the versions specified in the repository's [README.md](https://github.com/tristanmontoya/hydrolearn-hpc/blob/main/README.md) file to ensure compatibility with the provided scripts.

Two example scenarios are provided to illustrate different types of computational demand in hydrologic modeling workflows. You may complete one or both of them depending on your interests or research needs, but both assess the same underlying HPC concepts. The scenarios consist of the following simulation workflows, both of which involve predicting streamflow at the `CAN_05BB001` gauge on the Bow River at Banff, Alberta, Canada, by modeling the 2,216 km² snow-dominated watershed upstream of the gauge:

- **[Scenario 1](https://github.com/tristanmontoya/hydrolearn-hpc/blob/main/bow_at_banff_distributed_execution/ASSIGNMENT.md):** A distributed hydrologic model workflow, where computational cost arises from a single SUMMA-mizuRoute simulation that can be parallelized across spatial units.
- **[Scenario 2](https://github.com/tristanmontoya/hydrolearn-hpc/blob/main/bow_at_banff_lumped_calibration/ASSIGNMENT.md):** A lumped model calibration workflow, where computational cost arises from running a large number of independent SUMMA simulations within OSTRICH.
