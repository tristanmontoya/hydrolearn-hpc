...
def process_command_line():
    ...
    parser = argparse.ArgumentParser(
        description='Script to concatenate summa outputs from multiple GRU subsets.'
    )
    parser.add_argument(
        'control_file', type=str, nargs='?', help='path of the overall control file.'
    )
    parser.add_argument(
        '--summa-filemanager', type=str, help='path of the SUMMA fileManager.'
    )
...
def read_from_summa_route_config(config_file, setting):
    '''Function to extract a given setting from the summa or mizuRoute config.'''
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        print("   or: %s --summa-filemanager <fileManager.txt>" % sys.argv[0])
        sys.exit(0)
    # Otherwise continue
    args         = process_command_line()    

    # Identify the SUMMA fileManager from either a direct path or a control file.
    if args.summa_filemanager:
        summa_filemanager = args.summa_filemanager
        ...
        summa_settings_path = os.path.join(
            model_path, read_from_control(control_file, 'summa_settings_relpath')
        )
        summa_filemanager = os.path.join(
            summa_settings_path, read_from_control(control_file, 'summa_filemanager')
        )
    else:
        print("Usage: %s <control_file>" % sys.argv[0])
        print("   or: %s --summa-filemanager <fileManager.txt>" % sys.argv[0])
    ...
    merged_output_file = os.path.join(outputPath, outFilePrefix + '_day.nc')
    ...
    with nc.Dataset(outfilelist[0]) as src:
        ...
        # Store dimensioned variable names and their concatenation axis
        gru_vars = []
        hru_vars = []
        ...
        for j in range(gru_vars_num):
            ...
            if dim_index == 0:
                ...
                print('Variable %s has more than two dimensions: time and gru.'
                      % gru_var_name)
                sys.exit()
        for j in range(hru_vars_num):
            hru_var_name = hru_vars[j][0]
            ...
            if dim_index == 0:
                ...
                print('Variable %s has more than two dimensions: time and hru.'
                      % gru_var_name)
                sys.exit()
        ...
        with nc.Dataset(merged_output_file, "w") as dst:
            ...
            for name, dimension in src.dimensions.items():
                if name == 'gru':
                    ...
                    dim_size = len(dimension) if not dimension.isunlimited() else None
                    dst.createDimension(name, dim_size)
            ...
            for name, variable in src.variables.items():
                ...
                # Dimension names match, but gru and hru sizes are updated
